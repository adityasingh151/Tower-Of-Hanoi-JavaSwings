...
public class Tower extends JPanel implements MouseListener, MouseMotionListener {
	...
	public static void music() {
		try {
			AudioInputStream audioInputStream = AudioSystem
					.getAudioInputStream(new File("sounds/displace.wav").getAbsoluteFile());
	...
	public void init(int level) {
		...
		Color pegColor[] = { Color.GREEN, Color.MAGENTA, Color.yellow, Color.ORANGE, Color.blue, Color.BLACK,
				Color.CYAN, Color.pink, Color.RED, Color.lightGray };
	...
	public void mouseReleased(MouseEvent ev) {
		if (top != null && dragable == true) {
			...
			if (!stack[tower].empty()) {
				if (stack[tower].peek().getWidth() > top.getWidth()) {
					...
					JOptionPane.showMessageDialog(this, "You can't place bigger disc above smaller one", "Tower Of Hanoi", JOptionPane.ERROR_MESSAGE);
		...
		if (Integer.parseInt(levno.getText()) >= 10) {
			JOptionPane.showMessageDialog(this, "Congratulation !! You Complete all the level!!! ", "Tower of Hanoi",
